...
for (let a=1980; a < 2022; a++){
    years.push(a.toString().slice(-2)); //only push last two digits of year to years list
}

for(let i=0; i<years.length; i++){
    for(let z=0; z<months.length; z++){
        datelist.push(months[z] + '-' + years[i]);  //generate list from zipping months and years lists
    }
}
...
app.get('/api/all', (req, res) => {
    ...
    //generate blank output array
    for(var i = 0; i < tables_list.length; i++){
        output[tables_list[i]]=[];
    }
    ...
    db.all('SELECT * FROM cushing_1',[],(err, rows ) => {
        if (err) {throw err;}
            rows.forEach((row) => {output.cushing_1.push(row);}); 

            db.all('SELECT * FROM cushing_2',[],(err, rows ) => {
                if (err) {throw err;}
                    rows.forEach((row) => {output.cushing_2.push(row);}); 

                    db.all('SELECT * FROM cushing_3',[],(err, rows ) => {
                        if (err) {throw err;}
                            rows.forEach((row) => {output.cushing_3.push(row);}); 

                            db.all('SELECT * FROM harbor_1',[],(err, rows ) => {
                                if (err) {throw err;}
                                    rows.forEach((row) => {output.harbor_1.push(row);}); 

                                    db.all('SELECT * FROM harbor_2',[],(err, rows ) => {
                                        if (err) {throw err;}
                                            rows.forEach((row) => {output.harbor_2.push(row);}); 

                                            db.all('SELECT * FROM harbor_3',[],(err, rows ) => {
                                                if (err) {throw err;}
                                                    rows.forEach((row) => {output.harbor_3.push(row);}); 
